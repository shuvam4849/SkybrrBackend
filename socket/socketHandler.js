  ...
  try {
    ...
    socket.on('new message', async (newMessage) => {
      try {
        ...
        // ✅ PROCESS GROUPED MEDIA IF PRESENT
        let groupedMedia = [];
        if (newMessage.groupedMedia && Array.isArray(newMessage.groupedMedia)) {
          ...
          console.log(`✅ Processed ${groupedMedia.length} media items for grouped message`);
        }

        // ✅ Create message with groupedMedia
        let message = await Message.create({
          ...
          groupedMedia: groupedMedia, // ✅ Store grouped media
          status: 'sent',
          deliveredTo: [],
          readBy: []
        });
        ...
        // ✅ Format message for frontend - INCLUDE groupedMedia
        const formattedMessage = {
          _id: message._id,
          content: message.content,
          sender: {
          ...
          groupedMedia: message.groupedMedia, // ✅ Include in response
        ...
        if (connectedParticipants.length === 0) {
          console.log(`📴 All recipients offline for message ${message._id}, marking as pending`);
        ...
        console.log('✅ Message saved:', message._id, 'Status:', message.status, 'Grouped media:', groupedMedia.length);
      } catch (error) {
